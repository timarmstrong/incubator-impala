...
class RleBatchDecoder {
 public:
  ...
  bool DecodeLiteralValues(
      uint32_t num_literals_to_consume, OutType* dict, int64_t dict_len, OutType* values);

  /// Convenience method to get the next value. Not efficient.
  bool GetSingleValue(T* val);
 ...
 private:
  ...
  /// Called when both 'literal_count_' and 'repeat_count_' have been exhaustive.
  ...
  /// Output buffered literals and decrement 'literal_count_'.
  uint32_t OutputBufferedLiterals(uint32_t max_to_output, T* values);

  /// Output buffered literals and decrement 'literal_count_'. Return 0 if a decoding
  /// error is encountered.
  template <typename OutType>
  uint32_t DecodeBufferedLiterals(
      uint32_t max_to_output, OutType* dict, int64_t dict_len, OutType* values);
};
...
template <typename T>
inline uint32_t RleBatchDecoder<T>::NextNumRepeats() {
...
template <typename T>
inline T RleBatchDecoder<T>::GetRepeatedValue(uint32_t num_repeats_to_consume) {
...
template <typename T>
inline uint32_t RleBatchDecoder<T>::NextNumLiterals() {
...
inline bool RleBatchDecoder<T>::GetLiteralValues(
    uint32_t num_literals_to_consume, T* values) {
  ...
  uint32_t num_remaining = num_literals_to_consume - num_consumed;
  ...
  uint32_t num_to_bypass = std::min<uint32_t>(literal_count_,
  ...
  if (num_remaining > 0) {
    ...
    uint32_t num_copied = OutputBufferedLiterals(num_remaining, values + num_consumed);
...
inline bool RleBatchDecoder<T>::DecodeLiteralValues(
  ...
  // Need to round to a batch of 32 if the caller is consuming only part of the current
  // run avoid ending on a non-byte boundery.
  uint32_t num_to_bypass =
      std::min<uint32_t>(literal_count_, BitUtil::RoundDownToPowerOf2(num_remaining, 32));
  if (num_to_bypass > 0) {
...
inline void RleBatchDecoder<T>::NextCounts() {
  ...
  if (is_literal) {
    ...
    if (UNLIKELY(literal_count_ == 0)) return;
  } else {
    uint32_t repeat_count = indicator_value >> 1;
...
template <typename T>
inline uint32_t RleBatchDecoder<T>::OutputBufferedLiterals(
    uint32_t max_to_output, T* values) {
  uint32_t num_to_output =
      std::min<uint32_t>(max_to_output, num_buffered_literals_ - literal_buffer_pos_);
...
template <typename T>
template <typename OutType>
inline uint32_t RleBatchDecoder<T>::DecodeBufferedLiterals(
    uint32_t max_to_output, OutType* dict, int64_t dict_len, OutType* values) {
  uint32_t num_to_output =
      std::min<uint32_t>(max_to_output, num_buffered_literals_ - literal_buffer_pos_);
  for (uint32_t i = 0; i < num_to_output; ++i) {
