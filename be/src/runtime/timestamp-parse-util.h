...
class TimestampParser {
 public:
  ...
  // str -- pointer to the string to be parse
  // str_end -- the pointer to the end of the string to be parse
  ...
  static const char* ParseSeparatorToken(const char* str, const char* str_end, const char sep);

  /// Parse the date/time string to generate the DateTimeFormatToken required by
  /// DateTimeFormatContext. Similar to ParseFormatTokens() we will take the string and
  /// length, then via heuristic determine whether the value contains date tokens, time
  /// tokens, or both. Unlike ParseFormatTokens, we do not require the template format
  /// string.
