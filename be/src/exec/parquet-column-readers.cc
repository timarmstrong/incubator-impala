...
Status ParquetLevelDecoder::CacheNextBatch(int vals_remaining) {
  /// Fill the cache completely if there enough values remaining.
 ...
 protected:
  ...
  bool ReadValueBatch(MemPool* RESTRICT pool, int max_values, int tuple_size,
    ...
    while (val_count < max_values && !RowGroupAtEnd() && continue_execution) {
      ...
      // Fill the rep level cache if needed. We only need the repetition levels for
      // populating the position slot since we are only populating top-level tuples.
