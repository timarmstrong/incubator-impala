...
DEFINE_int32(partitioned_topn_in_mem_partitions_limit, 1000, "(Experimental) Soft limit "
    "on the number of in-memory partitions in an instance of the the partitioned top-n "
    "operator.");

// Soft limit on the aggregate size of heaps. If heaps exceed this, we will evict some
// from memory.
DEFINE_int64(partitioned_topn_soft_limit_bytes, 64L * 1024L * 1024L, "(Experimental) Soft "
    "limit on the number of in-memory partitions in an instance of the the partitioned "
    "top-n operator.");
...
Status TopNPlanNode::Init(const TPlanNode& tnode, FragmentState* state) {
  ...
  DCHECK(!tnode.sort_node.include_ties || is_partitioned())
      << "Only partitioned TopN handles ties";
...
void TopNPlanNode::Codegen(FragmentState* state) {
  ...
  if (codegen_status.ok()) {
    ...
    if (codegen_status.ok()) {
      ...
      if (is_partitioned()) {
        // The total number of calls to intra_partition_order_cmp_->Compare()
        // is 7 in PriorityQueue and 2 in TopNNode::Heap::InsertMaterializedTuple().
        // Each intra_partition_order_cmp_->Less(Tuple*, Tuple*) indirectly calls
        // Compare() once.
        replaced = codegen->ReplaceCallSites(insert_batch_fn,
            intra_partition_compare_fn, TupleRowComparator::COMPARE_SYMBOL);
        DCHECK_REPLACE_COUNT(replaced, 9) << LlvmCodeGen::Print(insert_batch_fn);
      } else {
        // The total number of calls to order_cmp_->Compare() is 3 in PriorityQueue and
        // 1 in TopNNode::Heap::InsertTupleRow().
        // Each order_cmp_->Less(Tuple*, Tuple*) indirectly calls Compare() once.
        replaced = codegen->ReplaceCallSites(insert_batch_fn,
...
vector<unique_ptr<TopNNode::Heap>> TopNNode::SelectPartitionsToEvict() {
  ...
  // just be reclaiming unreferenced variable-length data. We do not do that yet
...
Status TopNNode::PrepareForOutput(RuntimeState* state) {
  if (is_partitioned()) {
    ...
    DCHECK(!include_ties()) << "Unpartitioned Top-N doesn't support ties";
    DCHECK(heap_->DCheckConsistency());
    DCHECK_LE(heap_->num_tuples(), unpartitioned_capacity());
...
bool TopNNode::Heap::DCheckConsistency() {
  ...
  if (!overflowed_ties_.empty()) {
    ...
    DCHECK_EQ(capacity_, priority_queue_.Size())
        << "Ties should only be present in addition to an at-capacity heap";
  }
  return true;
}
...
Status TopNNode::ReclaimTuplePool(RuntimeState* state) {
  ...
  for (auto& entry : partition_heaps_) {
    ...
    // The key references memory in 'tuple_pool_'. Replace it with a rematerialized tuple.
