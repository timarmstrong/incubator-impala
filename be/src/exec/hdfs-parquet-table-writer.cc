...
class HdfsParquetTableWriter::BaseColumnWriter {
 ...
 protected:
  ...
  // create a new page and try again with the same value.
  ...
  int64_t num_values_; // Total number of values across all pages, including NULL.
  int64_t total_compressed_byte_size_;
  int64_t total_uncompressed_byte_size_;
  Encoding::type current_encoding_; // Encoding of the current page.
  Encoding::type next_page_encoding_; // Preferred encoding for the next page.
...
Status HdfsParquetTableWriter::BaseColumnWriter::FinalizeCurrentPage() {
  ...
  if (compressor_.get() == nullptr) {
    ...
    header.compressed_page_size = max_compressed_size;
    RETURN_IF_ERROR(compressor_->ProcessBlock32(true, header.uncompressed_page_size, uncompressed_data,
