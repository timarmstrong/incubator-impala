...
public class AnalyticPlanner {
  ...
  public PlanNode createSingleNodePlan(PlanNode root,
    ...
    List<Expr> assignedPreds = new ArrayList<Expr>();
    for (int i = 0; i < partitionGroups.size(); ++i) {
      ...
      for (int j = 0; j < partitionGroup.sortGroups.size(); ++j) {
        ...
        root = createSortGroupPlan(root, partitionGroup.sortGroups.get(j),
            ...
            lastSortGroup ? perPartitionLimits : null, assignedPreds);
      }
    }

    if (LOG.isTraceEnabled()) {
      LOG.trace("Assigned analytic conjuncts: " +
              Expr.listToSql(assignedPreds, ToSqlOptions.SHOW_IMPLICIT_CASTS));
    }
    analyticConjs.removeAll(assignedPreds);
    analyticConjs =
        Expr.substituteList(analyticConjs, root.getOutputSmap(), analyzer_, false);
    root = root.addConjunctsToNode(ctx_, analyzer_, tids, analyticConjs);
    return root;
  }
   ...
   * If successful, the assigned predicate is added to assignedPreds.
   */
  private PlanNode createSortGroupPlan(PlanNode root, SortGroup sortGroup,
      ...
      List<Expr> assignedPreds) throws ImpalaException {
    ...
    if (hasActivePartition || !isConstSort) {
      ...
      if(sortInfo.getSortTupleDescriptor().getSlots().size() > 0) {
        ...
        if (limit == null ||
          ...
          long planNodeLimit = limit.limit;
          if (limit.limit > 1) {
            if (limit.isLessThan) {
              // We do not need to evaluate a < or <= predicate after the analytic - the
              // top-n will filter the same rows as the predicate.
              assignedPreds.add(limit.conjunct);
            }
            planNodeLimit = limit.limit;
          } else {
            // The backend can't handle limits < 1. We need to apply a limit of 1, and
            // evaluate the predicate later.
            planNodeLimit = 1;
          }
          // Convert to standard top-N if not partitioning and no tie-handling required.
          if (partitionByExprs.isEmpty() && !limit.includeTies) {
  ...
  private List<PartitionLimit> inferPartitionLimits(List<Expr> conjuncts) {
    ...
    for (Expr conj : conjuncts) {
      ...
      if (AnalyticExpr.isAnalyticFn(fn, AnalyticExpr.RANK)) {
        // RANK() assigns equal values to . Thus if there are ties for the max RANK()
        // value to be returned, we need to return all of them. E.g. if the predicate
        // is RANK() <= 3, and the values we are ordering by are 1, 42, 99, 99, 100,
        // then the RANK() values are 1, 2, 3, 3, 5 and we need to return the top 4 rows:
