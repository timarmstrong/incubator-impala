...
public class DistributedPlanner {
 ...
 private DistributionMode computeJoinDistributionMode(JoinNode node,
    ...
    if (addedLowerTopN) {
      ...
      if (lowerTopN.isTypeTopN()) {
        upperTopN = SortNode.createTopNSortNode(ctx_.getQueryOptions(),
              ...
              lowerTopN.getSortInfo(), sortNode.getOffset(), lowerTopN.getLimit());
      } else {
  ...
  private PlanFragment createOrderByFragment(SortNode node,
    ...
    if (node.isPartitionedTopN() && node.isIncludeTies()) {
      // Partitioned TopN that returns needs special handling because ties are not
      ...
      Preconditions.checkState(!node.isIncludeTies(), "only part. top-n handles ties");
